;;
;; This script is ultimately run from GitHub Actions release.yml
;;
...
(defn- last-release-tag []
  (->>  (shell/command {:out :string}
        ...
        :out
        string/split-lines
        ;; Current tagging scheme ex: Release-1.2.3
        (filter #(re-matches #"Release-\d+\..*" %))
        first))
                  ...
                  (str
                    ...
                    (when last-version
                      (str
                        ...
                        last-version
                        "\\\\...Release-"  ;; single backslash is escape for AsciiDoc
                        version
                        "[Full commit log]\n\n"))
...
(defn- commit-changes! [version]
  (let [tag-version (str "Release-" version)]
    ...
    (shell/command "git commit -m" (str  "Release job: updates for " tag-version))
    (status/line :detail "Version tagging")
    (shell/command "git" "tag" "-a" tag-version "-m" tag-version)
...
;;
;; cmds dispatched form command line args
;;
...
(def ^:private usage (string/trim "
Valid args: (prep|deploy-remote|commit|validate|--help)
...
(defn- cmd-print-help [_]
  (println usage))

(defn- cmd-usage-error [_]
  (status/die 1 usage))

(defn -main [& args]
  (cli/dispatch
    [{:cmds ["prep"] :fn cmd-prep}
     {:cmds ["deploy-remote"] :fn cmd-deploy-remote}
     {:cmds ["commit"] :fn cmd-commit}
     {:cmds ["validate"] :fn cmd-validate}
